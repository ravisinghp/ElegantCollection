...
class UsersRepository(BaseRepository):
    ...
    async def get_user_by_id(self, user_id: int) -> MasterUser:
        """
        Required by get_current_user in authentication.py to validate tokens.
        """
        user_row = await self._log_and_fetch_one(GET_USER_BY_ID_QUERY, user_id)
        if user_row:
            # MANUAL MAPPING: Database Columns -> Pydantic Fields
            return MasterUser(
                id=user_row['user_id'],          
                username=user_row['user_name'],  
                email=user_row['mail_id'],       
